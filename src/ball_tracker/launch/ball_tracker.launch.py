...
def generate_launch_description():
    ...
    params_file_dec = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            get_package_share_directory("ball_tracker"), "config", "ball_tracker_params_example.yaml"
        ),
    ...
    follow_only_dec = DeclareLaunchArgument(
        "follow_only",
        default_value="false",
        description="Doesn't run the detect component. Useful for testing. (e.g. with manually published detections)",
    )

    tune_detection = LaunchConfiguration("tune_detection")
    tune_detection_dec = DeclareLaunchArgument(
        "tune_detection", default_value="false", description="Enables tuning mode for the detection"
    )

    use_sim_time = LaunchConfiguration("use_sim_time")
    use_sim_time_dec = DeclareLaunchArgument(
        "use_sim_time", default_value="false", description="Enables sim time for the follow node."
    )

    image_topic = LaunchConfiguration("image_topic")
    image_topic_dec = DeclareLaunchArgument(
        "image_topic", default_value="/my_camera/image_raw", description="The name of the input image topic."
    )

    cmd_vel_topic = LaunchConfiguration("cmd_vel_topic")
    cmd_vel_topic_dec = DeclareLaunchArgument(
        "cmd_vel_topic", default_value="/cmd_vel", description="The name of the output command vel topic."
    )

    enable_3d_tracker = LaunchConfiguration("enable_3d_tracker")
    enable_3d_tracker_dec = DeclareLaunchArgument(
        "enable_3d_tracker", default_value="false", description="Enables the 3D tracker node"
    )
